...
void RustyBucketEvent::trigger(character& c)
{
	...
	{
		//function to call the Mrs. Clean Dialogue
	}
	...
	else if (choice == "4") //Drink Stuff
	{
		...
		std::cout << ""; color(8); std::cout << "[3]: "; color(7); std::cout << "MudWiser" << std::endl;
		...
		if (choice == "1") //Barnacle Berry Blast
		{
			...
			std::cout << "You grab the mudwiser from the shop..." << std::endl;
			MSdelay(2000);
			std::cout << "It's a delicious mudwiser!" << std::endl;
			Dotdot();
